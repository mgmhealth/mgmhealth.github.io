...
(function($){
  Drupal.behaviors.bannerSlideshow = {
    ...
    attach: function() {
      $('#banner-widget > .content .slideshow-wrapper > .slides').html('');
      $('#banner-widget > .content .slideshow-wrapper > .slide-control .bullets-control > ul.bullets-list').html('');
      this.stance = Drupal.settings.design.is_sitemanager ? 'active' : 'published';

      this._globals.theme = Drupal.settings.design.theme;

      if(Drupal.settings.design.page[this.stance]['#banner'] == null
        || Drupal.settings.design.page[this.stance]['#banner'].type != 'slideshow'
        || $('#banner-widget').length == 0) {
        return false;
      }

      if(typeof Drupal.settings.design.page[this.stance]['#banner'].slideshow != 'undefined'
        && typeof Drupal.settings.design.page[this.stance]['#banner'].slideshow.settings.items != 'undefined') {

        if(typeof Drupal.settings.design.page[this.stance]['#banner'].slideshow.settings.items.obj != 'undefined'){
          this._globals.slides = [Drupal.settings.design.page[this.stance]['#banner'].slideshow.settings.items];
        } else {
          this._globals.slides = Drupal.settings.design.page[this.stance]['#banner'].slideshow.settings.items.slice();
        }
      }
      if(typeof this._globals.slides != 'undefined' && this._globals.slides != null) {
        if(document.readyState === "complete" || document.readyState === "interactive"){
          Drupal.behaviors.bannerSlideshow.loadSlides();
          Drupal.behaviors.bannerSlideshow.initEvents();
        } else {
          $(window).load(function() {
            Drupal.behaviors.bannerSlideshow.loadSlides();
            Drupal.behaviors.bannerSlideshow.initEvents();
    ...
    initEvents : function(){
      var that = this;
      if(this._globals.slides.length > $('#banner-widget .slide-initialized').length){
        setTimeout(function(){
          that.initEvents();
        }, 20);
      } else {
        this._globals.slideshowLayout = Drupal.settings.design.page[this.stance]['#banner'].slideshow.settings['layout'];

        $.extend(this._globals.slideshowSettings,
          Drupal.settings.design.page[this.stance]['#banner'].slideshow.settings['effect']);

        this._globals.slideshowSettings.delay *= 1000;

        this.attachEvents();
        if(this._globals.slides.length > 1) {
          this.initSlideshowTimer();
        }
      }
    },

    positionImage : function(slideId){
      var
        slides = this._globals.slides,
        that = this, slideSelector,
        imgCSS = {
          'position': 'absolute',
          'display': 'block',
          'z-index': 1,
        ...
        img = $('#banner-widget .slide-' + slideId).find('img');
      ...
      if(Drupal.settings.design.default_banner_settings['min-width'] == 'FULL' && Drupal.settings.design.default_banner_settings['max-width'] == 'FULL'){
        imgCSS.width =  '100%';
        imgCSS.height = 'auto';
        $(slideSelector = '#banner-widget .slide-' + slideId).addClass('slide-initialized');
        img.css(imgCSS);

      } else {
        if((typeof slides[slideId].obj[that._globals.theme] != 'undefined' && slides[slideId].obj[that._globals.theme].height == 'auto') || typeof slides[slideId].obj[that._globals.theme] == 'undefined'){
          slideSelector = '#banner-widget .slide-' + slideId;

          imgCSS.width =  $(slideSelector).width();
          imgCSS.height = 'auto';
          img.css('height', '');
          img.css(imgCSS);
          ...
          if(img[0].complete){
            if(img.height() < $(slideSelector).height()) {
              imgCSS.height = $(slideSelector).height();
              imgCSS.width = 'auto';
              img.css(imgCSS);
              imgCSS.top = 0 + 'px';
              imgCSS.left = ($(slideSelector).width() - img.width()) /2 + 'px';
              imgCSS.height = img.height();
              imgCSS.width = img.width();
            } else {
              imgCSS.left = 0 + 'px';
              imgCSS.top = ($(slideSelector).height() - img.height()) /2 + 'px';
              imgCSS.height = img.height();
              imgCSS.width = img.width();
            }
            $(slideSelector).addClass('slide-initialized');
            img.css(imgCSS);

            if(slideId == 0){
              that.initSlidesStyle('#banner-widget .slide-' + slideId, 'display');
            } else {
              that.initSlidesStyle('#banner-widget .slide-' + slideId, 'none');
            }
          } else {
            if($.browser.msie || !!navigator.userAgent.match(/Trident\/7\./)){
              setTimeout(function(){
                that.positionImage(slideId);
              }, 10);

            } else {
              img.data('imgCSS', imgCSS);
              img.data('slideSelector', slideSelector);
              img.unbind('load').load(function(){
                var loadImgCSS = $(this).data('imgCSS');
                var loadedSlideSelector = $(this).data('slideSelector');
                $(loadedSlideSelector).css({
                  display:'block',
                  visibility: 'hidden'
                });
                ...
                $(this)
                  ...
                  .closest('.slide')
                    .show();
                if($(this).height() != 0){
                  if($(this).height() < $(loadedSlideSelector).height()) {
                    loadImgCSS.height = $(loadedSlideSelector).height();
                    loadImgCSS.width = 'auto';
                    $(this).css(loadImgCSS);
                    loadImgCSS.top = 0 + 'px';
                    loadImgCSS.left = ($(loadedSlideSelector).width() - $(this).width()) /2 + 'px';
                    loadImgCSS.height = $(this).height();
                    loadImgCSS.width = $(this).width();
                  } else {
                    loadImgCSS.left = 0 + 'px';
                    loadImgCSS.top = ($(loadedSlideSelector).height() - $(this).height()) /2 + 'px';
                    loadImgCSS.height = $(this).height();
                    loadImgCSS.width = $(this).width();
                  }
                  $(this).css(loadImgCSS);
                }
                $(loadedSlideSelector).addClass('slide-initialized');
                if(slideId == 0){
                  that.initSlidesStyle('#banner-widget .slide-' + slideId, 'display');
                } else {
                  that.initSlidesStyle('#banner-widget .slide-' + slideId, 'none');
          ...
          imgCSS.width = slides[slideId].obj[that._globals.theme].width + 'px';
          imgCSS.height = slides[slideId].obj[that._globals.theme].height + 'px';
          imgCSS.top = slides[slideId].obj[that._globals.theme].top + 'px';
          imgCSS.left = slides[slideId].obj[that._globals.theme].left + 'px';
          $(slideSelector = '#banner-widget .slide-' + slideId).addClass('slide-initialized');
          img.css(imgCSS);
        }
    ...
    loadSlides: function() {
      var slides = this._globals.slides,
          imgSrc,
          that = this,
          pos = true;

      if(typeof this.slideTpl == 'undefined'){
        this.slideTpl = $('.slideshow-wrapper .slide').clone();
      }

      $('.slideshow-wrapper .slide').remove();

      for(slide = 0; slide < slides.length; slide++) {
        if(typeof slides[slide] != 'undefined' && slides[slide].obj != null) {
          if(Drupal.settings.design.default_banner_settings['min-width'] == 'FULL' && Drupal.settings.design.default_banner_settings['max-width'] == 'FULL'){
            if(typeof slides[slide].obj[this._globals.theme] != 'undefined' && typeof slides[slide].obj[this._globals.theme].croppedSrc != 'undefined'){
              imgSrc = slides[slide].obj[this._globals.theme].croppedSrc;
            } else if(typeof slides[slide].obj.builderSrc != 'undefined'){
              imgSrc = slides[slide].obj.builderSrc;
            } else {
              if(slides[slide].obj.src.indexOf('/files/uploads/') != -1){
                imgSrc = slides[slide].obj.src.replace('/files/','/files/styles/sb_large/public/');
              } else if(slides[slide].obj.src.indexOf('/files/stock_images/') != -1){
                imgSrc = slides[slide].obj.src.replace('/files/','/files/styles/sb_large/storage/');
              }
            }
            if (that.stance == 'active') {
              imgSrc += '?no_cache_id=' + new Date().getTime();
            } if(that.stance == 'published' && typeof Drupal.settings.content_manager.active_revision != 'undefined'){
              imgSrc += '?r=' + Drupal.settings.content_manager.active_revision;
            }

            that.slideTpl.find('img').remove();
            that.slideTpl.find('a').css({
              width : '100%',
              height : '100%',
            ...
            if(typeof slides[slide].obj[this._globals.theme] != 'undefined' && typeof slides[slide].obj[this._globals.theme].croppedSrc != 'undefined' && slides[slide].obj[this._globals.theme].croppedSrc != null) {
              imgSrc = slides[slide].obj[this._globals.theme].croppedSrc;
              pos = false;
            } else {
              imgSrc = slides[slide].obj.src;
              if(typeof slides[slide].obj.builderSrc != 'undefined'){
                imgSrc = slides[slide].obj.builderSrc;
              } else {
                if(slides[slide].obj.src.indexOf('/files/uploads/') != -1){
                  imgSrc = slides[slide].obj.src.replace('/files/','/files/styles/sb_large/public/');
            ...
            if (that.stance == 'active') {
              imgSrc += '?no_cache_id=' + new Date().getTime();
            } else if(that.stance == 'published' && typeof Drupal.settings.content_manager.active_revision != 'undefined'){
              imgSrc += '?r=' + Drupal.settings.content_manager.active_revision;
            }

            that.slideTpl.find('img').attr('src', imgSrc);
          }

          $('.slideshow-wrapper .slide-control').append(that.slideTpl.clone());
          $('.slideshow-wrapper .slide-control .slide:last-child').addClass('slide-' + slide);
          if(pos){
            that.positionImage(slide);
          } else {
            $('#banner-widget .slide-' + slide).addClass('slide-initialized');
          }
          //Image
          // if(typeof slides[slide].obj[that._globals.theme] == 'undefined' || typeof slides[slide].obj[that._globals.theme].croppedSrc == 'undefined') {
          //   var imgCSS = {
          //     'position': 'absolute',
          //     'display': 'block',
          //     'z-index': 1,
          ...
          //     height : 'auto'
          //   };
          //   img = $('#banner-widget .slide-' + slide).find('img');

          //   if((typeof slides[slide].obj[that._globals.theme] != 'undefined' && slides[slide].obj[that._globals.theme].height == 'auto') || typeof slides[slide].obj[that._globals.theme] == 'undefined'){
          //     // $(slide).css('display', 'block');
          //     slideSelector = '#banner-widget .slide-' + slide;
          ...
          //             .show();
          //         if($(this).height() != 0){
          //           if($(this).height() < $(slideSelector).height()) {
          //             loadImgCSS.height = $(slideSelector).height();
          //             loadImgCSS.width = 'auto';
          //             $(this).css(loadImgCSS);
          //             loadImgCSS.top = 0 + 'px';
          //             loadImgCSS.left = ($(slideSelector).width() - $(this).width()) /2 + 'px';
          //             loadImgCSS.height = $(this).height();
          //             loadImgCSS.width = $(this).width();
          //           } else {
          //             loadImgCSS.left = 0 + 'px';
          //             loadImgCSS.top = ($(slideSelector).height() - $(this).height()) /2 + 'px';
          //             loadImgCSS.height = $(this).height();
          //             loadImgCSS.width = $(this).width();
          //           }
          //           $(this).css(loadImgCSS);
          ...
          //     }
          //   } else {
          //     imgCSS.width = slides[slide].obj[that._globals.theme].width + 'px';
          //     imgCSS.height = slides[slide].obj[that._globals.theme].height + 'px';
          //     imgCSS.top = slides[slide].obj[that._globals.theme].top + 'px';
          //     imgCSS.left = slides[slide].obj[that._globals.theme].left + 'px';
          //     img.css(imgCSS);
          //   }
          ...
          //Link
          // console.log(Drupal.settings.preview);
          if(typeof slides[slide].link != 'undefined' && slides[slide].link.href.length > 0) {
            if (Drupal.settings.preview == true && Drupal.settings.previewm != true) {
              if (slides[slide].link.href === '/') {
                slides[slide].link.href = '/preview/';
              } else {
                slides[slide].link.href = slides[slide].link.href.replace('page/', 'preview/');
              }
            }
            if (Drupal.settings.previewm == true) {
              if (slides[slide].link.href === '/') {
                slides[slide].link.href = '/previewm/';
              } else {
                slides[slide].link.href = slides[slide].link.href.replace('page/', 'previewm/');
              }
            }
            $('.slideshow-wrapper .slide-control .slide-' + slide).find('.read-more')
              .attr('href', slides[slide].link.href)
              .attr('target', (typeof slides[slide].link.target != 'undefined' ? slides[slide].link.target : '_self'));
          } else {
            $('.slideshow-wrapper .slide-control .slide-' + slide).find('.read-more')
              .attr('href', '#')
              .removeAttr('target');
          ...
          that.initSlidesStyle('#banner-widget .slide-' + slide, 'none');

          Drupal.behaviors.bannerSlideshow._globals.slidesNr++;
        }
      }

      that.initSlidesStyle('#banner-widget .slide-0', 'block');
    },

    attachEvents: function() {
      //Slideshow controls
      switch(this._globals.slideshowLayout) {
        case 'slideshow_layout1':
          //previous button
          $('.slideshow-wrapper .slide-control .previous').click(this, function(e) {
            //directions
            var direction = '';
            if(e.data._globals.slideshowSettings.effect.indexOf('slide') != -1) {
              direction = e.data._globals.slideshowSettings.effect.replace('slide', '').toLowerCase();
            }
            ...
            if(e.data._globals.controlsActive) {
              e.data.stopSlideshowTimer();
              e.data.changeSlide(e.data._globals.curentSlide - 1, direction);
              e.data.initSlideshowTimer();
            }
          });
          //next button
          $('.slideshow-wrapper .slide-control .next').unbind('click').click(this, function(e) {
            //directions
            var direction = '';
            if(e.data._globals.slideshowSettings.effect.indexOf('slide') != -1) {
              direction = e.data._globals.slideshowSettings.effect.replace('slide', '').toLowerCase();
          ...
          break;
        case 'slideshow_layout2':
          // Put the bullets
          var bulletStructure = '<li class="bullet"><span></span></li>';

          for(var bullet = 0; bullet < this._globals.slides.length; bullet++) {
            $('.slideshow-wrapper .slide-control .bullets-control .bullets-list').append(bulletStructure);
            $('.slideshow-wrapper .slide-control .bullets-control .bullets-list .bullet:last').addClass('bullet-' + bullet);
          }
          $('.slideshow-wrapper .slide-control .bullets-control .bullets-list .bullet:first').addClass('active');

          var bulletWidth = parseInt($('.slideshow-wrapper .slide-control .bullets-control .bullets-list .bullet').outerWidth(true));
          $('.slideshow-wrapper .slide-control .bullets-control .bullets-list').width(bulletWidth * this._globals.slides.length);

          $('.slideshow-wrapper .slide-control .bullets-control .bullet').click(this, function(e) {
            if(!$(this).is('.active')) {
              var bulletClasses = $(this).attr('class').split(' '),
                  slideNr = parseInt(bulletClasses[1].replace('bullet-', ''));
              if(e.data._globals.controlsActive) {
                e.data.stopSlideshowTimer();
                //e.data._globals.curentSlide = slideNr;
                e.data.changeSlide(slideNr);
                e.data.initSlideshowTimer();
              }
            }
          });
    ...
    /*updateSlide: function(slide, index, data) {
      $(slide).find('img').remove();
      $('.buff-img-' + index).clone().prependTo(slide.find('a.read-more'));
      ...
      if(data.obj.src.length > 0) {
        if(typeof data.obj[that._globals.theme] != 'undefined' && typeof data.obj[that._globals.theme].croppedSrc != 'undefined'){
          img.attr('src', data.obj[that._globals.theme].croppedSrc);
        } else {
          img.attr('src', data.obj.src);
          if((typeof data.obj[that._globals.theme] != 'undefined' && data.obj[that._globals.theme].height == 'auto') || typeof data.obj[that._globals.theme] == 'undefined'){
            $(slide).css('display', 'block');
            imgCSS.width =  $(slide).width();
            imgCSS.height = 'auto';
            img.css('height', '');
            img.css(imgCSS);
            if(img.height() == 0){
              img.unbind('load').load(function(){
                if(img.height() != 0){
                  ...
                  if(img.height() < $(slide).height()) {
                    imgCSS.height = $(slide).height();
                    imgCSS.width = 'auto';
                    img.css(imgCSS);
                    imgCSS.top = 0 + 'px';
                    imgCSS.left = ($(slide).width() - img.width()) /2 + 'px';
                    imgCSS.height = img.height();
                    imgCSS.width = img.width();
                  } else {
                    imgCSS.left = 0 + 'px';
                    imgCSS.top = ($(slide).height() - img.height()) /2 + 'px';
                    imgCSS.height = img.height();
                    imgCSS.width = img.width();
                  }
                  img.css(imgCSS);
            ...
            imgCSS.width = data.obj[that._globals.theme].width + 'px';
            imgCSS.height = data.obj[that._globals.theme].height + 'px';
            imgCSS.top = data.obj[that._globals.theme].top + 'px';
            imgCSS.left = data.obj[that._globals.theme].left + 'px';
            img.css(imgCSS);
          }
        }
        // $(slide).find('img').css({
        //   'width': data.obj[that._globals.theme].width + 'px',
        //   'height': data.obj[that._globals.theme].height + 'px',
        //   'top': data.obj[that._globals.theme].top + 'px',
        //   'left': data.obj[that._globals.theme].left + 'px',
        // });
      }
      else {
        img.hide();
      ...
      if(data.title.text.length > 0) {
        $(slide).find('.slide-title').css({
          'position': 'absolute',
          'display': 'block',
          'top': data.title.top + 'px',
          'left': data.title.left + 'px',
          'width': (Math.ceil(data.title.width) + 3) + 'px',
          'height': (Math.ceil(data.title.height) + 3) + 'px',
          'color' : (data.title.color != undefined) ? data.title.color : '',
          'background-color' : (data.title.bgcolor != undefined) ? Drupal.hex2rgb(data.title.bgcolor, data.title.opacity) : '',
          'font-size' : (data.description.fontSize != undefined) ? data.title.fontSize + 'px' : '',
          'z-index': 2,
        });
        $(slide).find('.slide-title').text(data.title.text);
      }
      else {
        $(slide).find('.slide-title').hide();
      }

      if(data.description.text.length > 0) {
        $(slide).find('.slide-description').css({
          'position': 'absolute',
          'display': 'block',
          'top': data.description.top + 'px',
          'left': data.description.left + 'px',
          'width': (Math.ceil(data.description.width) + 3) + 'px',
          'height': (Math.ceil(data.description.height) + 3) + 'px',
          'color' : (data.description.color != undefined) ? data.description.color : '',
          'background-color' : (data.description.bgcolor != undefined) ? Drupal.hex2rgb(data.description.bgcolor, data.description.opacity) : '',
          'font-size' : (data.description.fontSize != undefined) ? data.description.fontSize + 'px' : '',
          'z-index': 2,
        });
        $(slide).find('.slide-description').text(data.description.text);
      }
      else {
        $(slide).find('.slide-description').hide();
      }

      if(typeof data.link != 'undefined' && data.link.href.length > 0) {
    	  $(slide).find('.read-more')
          .attr('href', data.link.href)
          .attr('target', (typeof data.link.target != 'undefined' ? data.link.target : '_self'));
      } else {
        $(slide).find('.read-more')
          .attr('href', '#')
          .removeAttr('target');
    ...
    initSlidesStyle: function(slide, display) {
      $(slide).css({
        'position': 'absolute',
        'top': '0px',
        'left': '0px',
    ...
    initSlideshowTimer: function() {
      this._globals.slideshowTimer = setInterval(function() {Drupal.behaviors.bannerSlideshow.changeSlide( Drupal.behaviors.bannerSlideshow._globals.curentSlide + 1 );}, this._globals.slideshowSettings.delay);
    },

    stopSlideshowTimer: function() {
      clearInterval(this._globals.slideshowTimer);
    },

    // pause : function(){
    //   this._globals.paused = true;
    //   //clearInterval(this._globals.slideshowTimer);
    // },

    // resume : function(){
    //   this._globals.paused = false;
    //   //this.initSlideshowTimer();
    // },

    changeSlide: function(slideNr, direction) {
      if(Drupal.animationsPaused) {
        return;
      }

      this._globals.controlsActive = false;

      this._globals.lastSlide = this._globals.curentSlide;
      this._globals.curentSlide = slideNr;
      if(this._globals.curentSlide >= this._globals.slides.length) {
        this._globals.curentSlide = 0;
      }
      if(this._globals.curentSlide < 0) {
        this._globals.curentSlide = this._globals.slides.length - 1;
      }

      var effect;
      if(this._globals.slideshowSettings.effect == 'random') {
        var effects = [
          'slideDown',
          'slideUp',
          'slideLeft',
        ...
        if(effects[effectIndex].indexOf('slide') != -1) {
          effect = 'slide';
          direction = effects[effectIndex].replace('slide', '').toLowerCase();
        }
        else {
          effect = effects[effectIndex];
        }
      }
      else if(this._globals.slideshowSettings.effect.indexOf('slide') != -1) {
        effect = 'slide';
        if(typeof direction == 'undefined') {
          direction = this._globals.slideshowSettings.effect.replace('slide', '').toLowerCase();
        }
      }
      else {
        effect = this._globals.slideshowSettings.effect;
      }

      var
        currentS = $('#banner-widget .slide-' + this._globals.curentSlide),
        lastS = $('#banner-widget .slide-' + this._globals.lastSlide);
      ...
      switch(effect) {
        case 'slide':
          ...
          switch(direction) {
            case 'left':
              ...
              }, parseInt(this._globals.slideshowSettings.speed));

              currentS.animate({
                'top': '0px',
                'left': '0px'
              }, parseInt(this._globals.slideshowSettings.speed),function() {
                // $('.slide-' + Drupal.behaviors.bannerSlideshow._globals.lastSlide).css({
                //   'display': 'none',
                // });

                //Reactivate the controls
                Drupal.behaviors.bannerSlideshow._globals.controlsActive = true;
              });
            ...
            case 'right':
              ...
              break;
            case 'up':
              var slideHeight = currentS.height();
        ...
        case 'fade':
          ...
          currentS.fadeIn(parseInt(this._globals.slideshowSettings.speed));
          lastS.fadeOut(parseInt(this._globals.slideshowSettings.speed), function() {
            //Reactivate the controls
            Drupal.behaviors.bannerSlideshow._globals.controlsActive = true;
          });
      ...
      if(this._globals.slideshowLayout == 'slideshow_layout2') {
        //Change bullets
        $('.slideshow-wrapper .slide-control .bullets-control .bullet.active').removeClass('active');
        $('.slideshow-wrapper .slide-control .bullets-control .bullet.bullet-' + this._globals.curentSlide).addClass('active');
      }
    }
  }
}(jQuery));
